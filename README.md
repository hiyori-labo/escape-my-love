# 💕 Escape My Love

**恋愛脱出ゲーム** - 閉じ込められた部屋から脱出できるか、それとも愛に絆されてしまうか。

## 🎮 ゲーム概要

あなたは愛する人に閉じ込められた部屋で目を覚ます。
脱出を目指して探索するか、相手の甘い誘惑に身を委ねるか——選択はあなた次第。

### 2つのエンディング

- 🚪 **脱出成功** - 誘惑を振り切り、部屋から脱出する
- 💕 **永住確定** - 愛に絆され、ここに留まることを選ぶ

## ✨ 特徴

- **キャラクターカスタマイズ** - プレイヤー名、相手役の名前・性格・口調を自由に設定
- **AI生成シナリオ** - Gemini APIによる動的なストーリー展開
- **セーブ/ロード機能** - 最大3つのセーブスロット
- **ログ記録機能** - ターンごとやクリア後のエピローグのログをワンタップでコピー可能
- **PWA対応** - スマホのホーム画面に追加して遊べる

## 🚀 遊び方

### オンラインで遊ぶ

👉 **[Escape My Love を遊ぶ](https://escape-my-love.vercel.app/)**

### ローカルで遊ぶ

```bash
git clone https://github.com/hiyori-labo/escape-my-love.git
cd escape-my-love
vercel dev
# http://localhost:3000 をブラウザで開く
```

### 遊ぶためのステップ（1日15ターン制限）

ゲームは基本的に **1日15ターンまで無料** で遊ぶことができます（毎日0時にリセットされます）。
上限なしでプレイしたい場合は、以下の手順でご自身のAPIキーを設定してください。

1. **Gemini API Key を取得**
   - [Google AI Studio](https://aistudio.google.com/app/apikey) でAPIキーを取得

2. **ゲームにキーをセット**
   - メニュー（☰）→「🔑 APIキー設定」から取得したAPIキーを入力

3. **キャラクターを設定してスタート**
   - メニュー（☰）→「⚙️ キャラクター設定」で好みのキャラ・口調・舞台を設定して「New Game」

## 🛠️ 技術スタック

- HTML / CSS / JavaScript（Vanilla）
- Node.js（Vercel Serverless Functions）
- Gemini API（gemini-3-flash-preview）
- PWA（Progressive Web App）

## ⚠️ 免責事項

本アプリケーションは、Google Gemini APIを利用したテキスト生成ツールです。

- **本アプリケーションは18歳以上の方を対象としています。** 18歳未満の方のご利用はお断りします。これはGoogle Gemini APIの利用規約上の要件でもあります。
- 登場するキャラクターはAIが生成したフィクションであり、実在の人物・団体とは一切関係ありません。
- 生成されるテキストの内容について、開発者は一切の責任を負いません。AIの性質上、意図しない内容が生成される場合があります。
- 公序良俗に反する入力や、法に触れる用途での利用は固く禁じます。
- 利用者は、自身の責任において本アプリケーションを利用するものとします。
- 独自のAPIキーを設定して利用する場合は、[Google Gemini APIの利用規約](https://ai.google.dev/gemini-api/terms)に従うものとします。APIキーの管理は利用者自身の責任で行ってください。

🔒 データはすべてブラウザ内に保存され、開発者のサーバーには送信されません。

## 📝 ライセンス

MIT License

---

Made with 💕
